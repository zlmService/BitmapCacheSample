...
public class Main2Activity extends AppCompatActivity  implements AbsListView.OnScrollListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        initData();
        initView();
        mImageLoader=ImageLoader.build(Main2Activity.this);
    }
    ...
    private void initData() {
        ...
        //dp和px之间的转换（把20px转换成 dip单位）返回的值当做 图片之间的间距
    ...
    private class ImageAdapter extends BaseAdapter{
        ...
        public View getView(int position, View convertView, ViewGroup parent) {
            ...
            if (!uri.equals(tag)) {
                vh.imageView.setImageDrawable(mDefaultBitmapDrawable);
            }
            if (mIsGridViewIdle && mCanGetBitmapFromNetWork) {
                vh.imageView.setTag(uri);
                mImageLoader.bindBitmap(uri, vh.imageView, mImageWidth, mImageWidth);
            }
            return convertView;
        }
    }
